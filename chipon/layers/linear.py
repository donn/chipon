...
class Linear:
    @classmethod
    def from_quantized_pytorch_layer(cls, layer, index: int, input_scale: float):
        # print(dir(layer))
        weights_quantized = torch.int_repr(layer.weight())
        # TODO: quantize biases
        return cls(
            layer.in_features,
            layer.out_features,
            weights_quantized.numpy().T,
            layer.bias().detach().numpy(force=True),
            index,
        )

    @classmethod
    def from_pytorch_layer(cls, layer, index: int):
        return cls(
            layer.in_features,
            layer.out_features,
            layer.weight.detach().numpy().T,
    ...
    def __init__(
        ...
        self.name = (
            f"layer_{index}_linear_{str(self.in_features)}_{str(self.out_features)}"
        )
    ...
    def verify_weights(self):
        ...
        if self.weight.shape != weight_shape:
            raise ValueError(
                f"Weight shape is not correct, expected {weight_shape}, got {self.weight.shape}"
            )

        if self.bias is not None and self.bias.shape != bias_shape:
            raise ValueError(
                f"Bias shape is not correct, expected {bias_shape}, got {self.bias.shape}"
            )
